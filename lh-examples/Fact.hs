...
{-@ ackP :: m:_ -> n:_ -> _ / [plen m, plen n] @-}
ackP Z n         = S n
ackP (S m) Z     = ackP m (S Z)
ackP (S m) (S n) = ackP m (ackP (S m) n)
...
add_lid (S n) = add (S n) Z === S (add n Z) ? add_lid n === S n *** QED
...
add_comm Z n     = add Z n === n ? add_lid n === add n Z *** QED
add_comm (S m) n
  =   add (S m) n
  === S (add m n)
  ? add_comm m n
